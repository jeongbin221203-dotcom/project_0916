...
def _read_pdf(data: bytes) -> tuple[str, list[str]]:
    ...
    try:
        with pdfplumber.open(io.BytesIO(data)) as pdf:
            ...
            images = [_jpeg_data_url(page.to_image(resolution=150).original)
...
def _read_image(data: bytes) -> list[str]:
    from PIL import Image, UnidentifiedImageError

    try:
        with Image.open(io.BytesIO(data)) as image:
            image.load()
            return [_jpeg_data_url(image.copy())]
    except (UnidentifiedImageError, OSError, Image.DecompressionBombError):
        raise ServiceError("그림 파일을 열지 못했습니다. PNG나 JPG로 다시 저장해 올려 주세요.",
                           "VALIDATION_ERROR")


def read_upload(filename: str, data: bytes) -> tuple[str, list[str]]:
    """올린 파일에서 (본문 글자, 그림 data URL 목록)을 꺼냅니다."""
...
def extract(filename: str, data: bytes) -> dict:
    ...
    raw = _ask_ai(text, images)
    form = to_form(raw)
    notes = form.pop("notes")
