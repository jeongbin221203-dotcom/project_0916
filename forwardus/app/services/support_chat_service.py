...
MODE_WORDS = ("항공", "비행", "air", "해상", "배로", "선박", "sea", "ocean", "lcl", "fcl")


def _mode_note(current, text: str) -> str:
    """해상·항공을 어떻게 다룰지 AI에게 일러 주는 한 줄.

    세 갈래입니다.
      한 번도 안 말함            둘 다 나눠 설명하고 어느 쪽인지 물어봅니다
      이번 말에서 말함            그 한쪽으로만 답합니다
      전에 말했고 이번엔 안 말함   그 쪽을 기준으로 답하되 **다른 쪽도 견줍니다**

    마지막 갈래가 중요합니다. "항공으로 보내면"이라고 물은 뒤에 치수만 적으면,
    그 말에는 모드가 없습니다. 항공만 답하면 해상이 쌀 수도 있다는 것을 모르고,
    둘 다만 답하면 방금 항공이라 말한 사람에게 딴소리가 됩니다. (2026-09-26)
    """

    mode = (current or {}).get("운송")
    if not mode:
        return (" 운송 모드(해상/항공)는 아직 정해지지 않았습니다. "
                "**해상과 항공을 둘 다** 나눠 설명하고, 기간·비용·서류가 어떻게 "
                "다른지 짚어 주세요. 한쪽만 고르지 마세요. "
                "마지막에 어느 쪽으로 보내실지 물어보세요.")
    lowered = str(text or "").lower()
    if any(word in lowered for word in MODE_WORDS):
        return ""                      # 이번 말에서 짚었습니다. 그쪽으로만 답합니다.
    return (f" 이번 말에는 해상인지 항공인지가 없습니다. **{mode}**을(를) 기준으로 "
            "답하시되, 끝에 다른 쪽(해상↔항공)으로 보내면 기간·비용이 어떻게 "
            "달라지는지 **두세 줄로 견줘** 주세요. 기준을 바꾸라는 말은 아닙니다.")
